...
# -------------------------------------------
# App Title
st.title(" Stocks Portfolio Optimization (MVP + Tangency)")
...
def generate_efficient_frontier(expected_ret, cov_matrix, risk_free_rate):
    frontier_returns = np.linspace(min(expected_ret)*0.8, max(expected_ret)*1.5, 50)
    risks, returns = [], []
...
risks, returns = generate_efficient_frontier(expected_ret, cov_matrix, risk_free_rate)
